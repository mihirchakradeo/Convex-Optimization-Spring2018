...
def objective_scalar_constraints( constraints, x, order=0 ):
    ...
    for constraint in constraints:
        ...
        if order >= 1:
            ...
            # print("cv",constraint_value[0])
            # print("cg",constraint_gradient.shape)
            gradient = (-1/np.asscalar(constraint_value)) * (constraint_gradient)
            ...
            if order == 2:
                ...
                # hessian = ((1/(np.asscalar(constraint_value)*np.asscalar(constraint_value))) * (constraint_hessian)) - (constraint_hessian/np.asscalar(constraint_value))
                hessian = (constraint_gradient*constraint_gradient.T/(np.asscalar(constraint_value)*np.asscalar(constraint_value))) -  (constraint_hessian/np.asscalar(constraint_value))
