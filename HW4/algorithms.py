...
def subgradient_descent( func, initial_x, maximum_iterations=65536, initial_stepsize=1):
    ...
    while True:
        ...
        eta_t = initial_stepsize/np.sqrt(iterations+1)
        x = x - eta_t*gradient
...
def adagrad( func, initial_x, maximum_iterations=65536, initial_stepsize=1, initial_sum_of_squares=1e-3):
    ...
    while True:
        ...
        temp_v += np.square(gradient)
        eta_t = initial_stepsize/np.sqrt(initial_sum_of_squares + temp_v)
        x = x - np.multiply(eta_t, gradient)
